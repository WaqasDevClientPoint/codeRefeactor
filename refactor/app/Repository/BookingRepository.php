...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        $query = $this->applyUserRoleFilter($request,$query, $requestdata);
        //added pagination
        if ($limit == 'all') {
            $allJobs = $query->get();
        } else {
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
    ...
    //created helper function  to move common logic here
    private function applyUserRoleFilter($request,$query, $requestdata)
    {
        $cuser = $request->__authenticatedUser;

        if ($cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            // Apply superadmin-specific filters
            ...
            // Add more filters specific to superadmins
        } else {
            // Apply filters for other user roles
            if ($cuser->consumer_type == 'RWS') {
                $query->where('job_type', '=', 'rws');
            } else {
                $query->where('job_type', '=', 'unpaid');
            }
            // Add more filters for other roles
        }
